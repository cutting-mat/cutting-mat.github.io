...
CuttingMat的目录结构是以**业务模块拆分**为目标设计的。
...
基于以上目录结构，可以天然实现按业务模块拆分的代码组织能力，在多人协同项目中可以更好的开发解耦，避免代码冲突，配合git分支策略也很容易实现业务模块分批提测、分批上线。

对于项目整体来说，主模块是项目入口，提供所有业务模块的路由父节点，就像一个托盘一样托起所有业务模块，这是主模块与其他模块的根本性区别。

另外主模块还充当了项目的公共资源池，比如公共代码、公共组件等都在主模块里；还有项目里的公共路由，比如登录注册页面，都在主模块里实现。配合前端团队内的开发约定，可以更好的实现公共资源管理，减少项目内的造轮子，提高协同开发效率。

项目里还有一个`core/`文件夹，顾名思义这里是框架的核心功能实现，是框架工具能力的实体化，工具能力和组织能力共同构成CuttingMat的框架能力。
